...
public class UserConsumer {
    ...
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(value = "${animalerie-health.broker.queue.userEventQueue.name}", durable = "true"),
            exchange = @Exchange(value = "${animalerie-health.broker.exchange.userEventExchange}", type = ExchangeTypes.FANOUT, ignoreDeclarationExceptions = "true")
    ))
